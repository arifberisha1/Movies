...
export default function MovieDetails() {
  ...
  function handleRate(rate: number) {
    axios.post(urlRatings, { rating: rate, movieId: id }).then(() => {
      Swal.fire({ icon: "success", title: "Rating received" }).then(
        (result) => {
  ...
  async function AddComment(commentFormDto: commentFormDTO) {
    ...
    try {
      await axios
        .post(`${urlComments}/create`, commentCreationDto)
        ...
        <>
          ...
          <AddRemoveButtons
            ...
            removeButton={async () => {
              try {
                await axios
                  .delete(`${urlFavourite}/delete/${getEmail()}/${id}`)
                  .then((response: AxiosResponse) => {
                    Swal.fire({
                      title: "Success",
                      text: response.data,
          ...
          <AddRemoveButtons
            ...
            removeButton={async () => {
              try {
                await axios
                  .delete(`${urlWatched}/delete/${getEmail()}/${id}`)
                  .then((response: AxiosResponse) => {
                    Swal.fire({
                      title: "Success",
                      text: response.data,
          ...
          <div style={{ display: "flex", flexDirection: "column" }}>
            {movie.actors?.map((actor) => (
              <div key={actor.id} style={{ marginBottom: "2px" }}>
                ...
                <Link
                  to={`/actors/details/${actor.id}`}
                  className={"btn btn-dark ms-5 location-fixed"}
                >
                  More
                </Link>
              </div>
