...
public class MoviesSeedData
{
    public static async Task InitializeMovies(IServiceProvider serviceProvider)
    {
        using (var scope = serviceProvider.GetRequiredService<IServiceProvider>().CreateScope())
        {
            ...
            var hasMovies = await context.Movies.ToListAsync();

            if (hasMovies.Count == 0)
            {
                // Adding movies
                ...
                var moviesList = new List<Movie>
                {
                    new Movie
                    {
                        ...
                        Trailer =
                            "https://www.youtube.com/watch?v=KPLWWIOCOOQ&pp=ygUXZ2FtZSBvZiB0aHJvbmVzIHRyYWlsZXI%3D",
                        InTheaters = true,
                    ...
                    new Movie
                    {
                        ...
                        Trailer =
                            "https://www.youtube.com/watch?v=9POCgSRVvf0&pp=ygUbYWxpY2UgaW4gd29uZGVybGFuZCB0cmFpbGVy",
                        InTheaters = true,
                ...
                foreach (var moviesMovietheater in moviesMovitetheatersList)
                {
                    var movieTheaterExist = await context.MovieTheatersMovies.FirstOrDefaultAsync(x =>
                        x.MovieId == moviesMovietheater.MovieId &&
                        x.MovieTheaterId == moviesMovietheater.MovieTheaterId);
